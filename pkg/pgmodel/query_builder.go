...
const (
	...
	timeseriesByMetricSQLFormat = `SELECT (` + promSchema + `.label_array_to_kv(s.labels)).*, array_agg(m.time ORDER BY time), array_agg(m.value ORDER BY time)
	FROM %s m
	...
	timeseriesBySeriesIDsSQLFormat = `SELECT (` + promSchema + `.label_array_to_kv(s.labels)).*, array_agg(m.time ORDER BY time), array_agg(m.value ORDER BY time)
	FROM %s m
	INNER JOIN _prom_catalog.series s
	ON m.series_id = s.id
